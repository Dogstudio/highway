...
var _renderer = _interopRequireDefault(__webpack_require__(35));
...
function () {
  ...
  _createClass(Helpers, null, [{
    ...
    key: "getRenderer",
    value: function getRenderer(slug, renderers) {
      if (typeof renderers === 'undefined' || !renderers) {
        return _renderer.default;
      }

      return slug in renderers ? renderers[slug] : _renderer.default;
    }
    /**
     * Get page transition
     *
    ...
    key: "getTransition",
    value: function getTransition(slug, transitions) {
...
module.exports = {
  set: Object.setPrototypeOf || ('__proto__' in {} ? // eslint-disable-line
    function (test, buggy, set) {
      try {
        set = __webpack_require__(12)(Function.call, __webpack_require__(57).f(Object.prototype, '__proto__').set, 2);
        set(test, []);
...
var _tinyEmitter = _interopRequireDefault(__webpack_require__(64));
...
function (_Emitter) {
  ...
  function Core() {
    ...
    _this = _possibleConstructorReturn(this, _getPrototypeOf(Core).call(this)); // All your custom renderers and transitions you sent to Highway.
    ...
    window.addEventListener('popstate', _this.popState.bind(_assertThisInitialized(_assertThisInitialized(_this)))); // Event binding
  ...
  _createClass(Core, [{
    key: "getProps",
    value: function getProps(context) {
      var page = _helpers.default.getDOM(context);

      var view = _helpers.default.getView(page);

      var slug = _helpers.default.getSlug(view);

      var transition = _helpers.default.getTransition(slug, this.transitions);
    ...
    key: "getState",
    value: function getState(location) {
      return {
        url: location,
        anchor: _helpers.default.getAnchor(location),
        origin: _helpers.default.getOrigin(location),
        params: _helpers.default.getParams(location),
        pathname: _helpers.default.getPathname(location)
      };
    }
    /**
    ...
    value: function click(event) {
      ...
      event.preventDefault(); // Now get the URL of the target element!

      var href = event.currentTarget.href; // Check
    ...
    value: function check(url) {
      ...
      // event appropriately.
      var params = _helpers.default.getParams(url);

      var pathname = _helpers.default.getPathname(url);
      ...
      if (!this.navigating && (pathname !== this.state.pathname || params)) {
        ...
        if (pathname === this.state.pathname && params) {
          var oldParams = JSON.stringify(this.state.params);
        ...
        this.link = url; // Now push the state!

        this.pushState();
      } else {
        // Check if there are anchors
        var anchor = _helpers.default.getAnchor(url); // If the pathnames are the same there might be an anchor appended to
    ...
    value: function popState() {
      ...
      var state = this.getState(window.location.href);
    ...
    value: function pushState() {
      // We update the state based on the clicked link `href` property.
      var state = this.getState(this.link); // We push a new entry in the history in order to be able to navigate
      // with the backward and forward buttons from the browser.

      state.pathname && window.history.pushState(state, '', state.url); // Call `beforeFetch` for optimizations.
    ...
    value: function () {
          ...
          while (1) {
            switch (_context.prev = _context.next) {
              case 0:
                ...
                this.unbind(); // Update state with the one returne by the browser history. Basically
                // this is the state that was previously pushed by `history.pushState`.

                this.state = state; // We check cache to avoid unecessary HTTP requests.
              ...
              case 7:
                ...
                this.props = this.getProps(results[0]); // Cache page
      ...
      return function beforeFetch(_x) {
        ...
        while (1) {
          switch (_context2.prev = _context2.next) {
            ...
            case 5:
              ...
              this.emit('NAVIGATE_ERROR', response); // !200+: Error of the HTTP request
