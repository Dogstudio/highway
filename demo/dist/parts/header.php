<!-- Buttons -->
<!--p class="site-header-buttons">
    <a class="github-button" href="https://github.com/Dogstudio/highway/subscription" data-icon="octicon-eye" data-size="large" aria-label="Watch Dogstudio/highway on GitHub">Watch</a>
    <a class="github-button" href="https://github.com/Dogstudio/highway" data-icon="octicon-star" data-size="large" aria-label="Star Dogstudio/highway on GitHub">Star</a>
    <a class="github-button" href="https://github.com/Dogstudio/highway/fork" data-icon="octicon-repo-forked" data-size="large" aria-label="Fork Dogstudio/highway on GitHub">Fork</a>
</p-->

<!-- Banner -->
<a href="https://github.com/Dogstudio/highway" target="_blank"><img style="position: absolute; top: 0; right: 0; border: 0;" src="https://s3.amazonaws.com/github/ribbons/forkme_right_darkblue_121621.png" alt="Fork me on GitHub"></a>

<!-- Header -->
<header class="site-header">
    <div class="site-header-inner">
        <a href="/" class="site-logo"><img src="./assets/images/logo_2x.png" /></a>

        <nav class="site-menu">
            <ul>
                <li class="menu-item"><a href="/">Introduction</a></li>
                <li class="menu-item"><a href="/installation.php">Installation</a></li>
                <li class="menu-item">
                    <a href="/get-started.php">Get started</a>
                    <!--ul class="menu-child">
                        <li class="menu-item"><a href="/get-started.php#structure">Structure</a></li>
                        <li class="menu-item"><a href="/get-started.php#renderers">Renderers</a></li>
                        <li class="menu-item"><a href="/get-started.php#transitions">Transitions</a></li>
                        <li class="menu-item"><a href="/get-started.php#core">Core</a></li>
                    </ul-->
                </li>
                <li class="menu-item"><a href="/api.php">API</a></li>
                <li class="menu-item"><a href="/examples.php">Examples</a></li>
                <li class="menu-item"><a href="/sdsd.php">404</a></li>
                <li class="menu-item"><a href="https://github.com/Dogstudio/highway">Github</a></li>
            </ul>
        </nav>
    </div>
</header>
